...
def donor_home(request):
    ...
    for donor_request in donor_requests:
        ...
        try:
            appointment = Appointments.objects.filter(donation_request=donor_request).latest('id')
            donor_request.appointment_status = appointment.appointment_status
        except Appointments.DoesNotExist:
            donor_request.appointment_status = "Not Booked"
    
    return render(request, "donor-home.html", {
        "donationrequests": donor_requests,
...
def book_appointment(request):
    ...
    if request.POST:
        ...
        apmt = Appointments()
        donation_request = DonationRequests.objects.get(id=int(request.POST.get("dreq", "")))
        apmt.donation_request = donation_request
        ...
        # Update donation request status to "Booked"
        donation_request.donation_status = "Booked"
        donation_request.save()
